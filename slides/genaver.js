  ...
  for (const key of Object.keys(specs)) {
    ...
    switch (op) {
      case "=":
        ...
        break;
      case "!=":
        filter_str_diff(data_key, val);
        break;
      case "@":
        filter_num_at(data_key, val);
        break;
      case "<":
        filter_num(data_key, val, is_lt);
        break;
      case "<=":
        filter_num(data_key, val, is_le);
        break;
      case ">":
        filter_num(data_key, val, is_gt);
        break;
      case ">=":
        filter_num(data_key, val, is_ge);
        break;
      default:
        alert(`Unknown comparison '${op}'!`);
        break;
    }
  }
}
...
function is_ge(x, limit) {
  return x >= limit;
}
function is_gt(x, limit) {
  return x > limit;
}
function is_le(x, limit) {
  return x <= limit;
}
function is_lt(x, limit) {
  return x < limit;
}
